...
void MyModBus_init(MyModBus_t* this, const MyModbus_config_t* cfg)
{
    ...
    //periferiak initje
    if (cfg->peripheralsInitFunc)
    {
        cfg->peripheralsInitFunc(false, this->callbackData);
    }
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//Akkor hivodik, ha ujabb karaktert kapott. Kivulrol hivott fuggevny.
...
static void MyModBus_sendResponse(MyModBus_t* this)
{
    ...
    //soros vonalon...
    this->sendFunc(this->txFrame.buff, responseLength, this->callbackData);
}
//------------------------------------------------------------------------------
